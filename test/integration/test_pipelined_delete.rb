...
describe 'Pipelined Delete' do
  it 'supports pipelined delete' do
    memcached_persistent do |dc|
      ...
      assert_equal(true, result['a'])  # deleted
      assert_equal(true, result['b'])  # deleted
      assert_equal(true, result['c'])  # deleted
      assert_equal(false, result['f']) # not found
  ...
  it 'handles non-existent keys' do
    memcached_persistent do |dc|
      ...
      assert_equal(false, result['y'])
      assert_equal(false, result['z'])
    end
  end
  ...
  it 'supports pipelined delete with keys containing Unicode' do
    memcached_persistent do |dc|
      ...
      assert_equal(true, result['a'])
      assert_equal(true, result[encoded_key])
  ...
  it 'performs large batch delete efficiently' do
    memcached_persistent do |dc|
      ...
      delete_result = dc.delete_multi(keys_to_delete)

      # Verify deletion results
      keys_to_delete.each do |key|
        assert_equal(true, delete_result[key], "Expected #{key} to be deleted")
      end
  ...
  it 'handles mixed success and failure gracefully' do
    memcached_persistent do |dc|
      ...
      assert_equal(true, result['exists1'])
      assert_equal(false, result['notexist1'])
      assert_equal(true, result['exists2'])
      assert_equal(false, result['notexist2'])
  ...
  it 'returns consistent results for repeated deletes' do
    memcached_persistent do |dc|
      dc.close
      dc.flush
      ...
      result1 = dc.delete_multi(['key'])

      assert_equal(true, result1['key'])

      # Second delete should indicate key not found
      result2 = dc.delete_multi(['key'])

      assert_equal(false, result2['key'])

      # Third delete should also indicate key not found
      result3 = dc.delete_multi(['key'])

      assert_equal(false, result3['key'])
    end
  end
end
